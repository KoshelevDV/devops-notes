...
## Антипаттерны алертов (и как их исправить)

> Источник: практика SRE Google + опыт VK/LinkedIn

### Метрика качества алертов

**Signal-to-Noise Ratio** = (actionable alerts) / (total alerts)
- ✅ Здоровая система: **30–50%** actionable
- ⚠️ Проблемная система: **<10%** actionable → рефакторинг алертов в приоритет

73% outage происходят из-за **проигнорированных** алертов — не из-за отсутствия мониторинга.

---

### Антипаттерн 1: Алертит всё подряд

**Симптом:** 2000+ алертов в неделю, но только 3% требуют действия.

**Решение — 4 Golden Signals (Google SRE):**
...
| **Saturation** | Насыщенность (CPU, память, диск) |

Этих четырёх достаточно для 80% проблем. Остальное — noise.

**Действие:** пройтись по всем алертам — если не про эти 4 сигнала, удалить или переделать.

---

### Антипаттерн 2: Нет severity

**Симптом:** все алерты одинаковые, 50 нотификаций в час, непонятно что критично.

**Решение — 4 уровня:**

| Уровень | Когда | Реакция |
|---------|-------|---------|
| **DISASTER** | Сервис упал, users affected | Звонок сейчас |
| **CRITICAL** | Сервис деградирует | Разобрать сегодня, можно за час |
| **HIGH** | Плохой тренд, может стать проблемой | Разобрать завтра, добавить в спринт |
| **WARNING** | Отслеживаем, пока OK | Смотри дашборд, не звони |

---

### Антипаттерн 3: Нет runbook

**Симптом:** алерт в 3 ночи, on-call смотрит на название и не понимает что делать → 1-2 часа поиска.

**Правило:** каждый DISASTER/CRITICAL алерт **обязан** иметь runbook.

**Структура runbook:**
1. **Что произошло** — человеческим языком, без жаргона
2. **Что проверить** — какие логи, какие метрики
3. **Как чинить** — пошаговые шаги
4. **Время** — должно решаться за 10 минут без escalation

> Если алерт не решается за 10 минут и нет инструкции — это не алерт, это шум.

---

### Антипаттерн 4: Нет владельца

**Симптом:** алерт срабатывает, никто не знает кто отвечает, тикет гуляет по сотрудникам.

**Решение:** у каждого алерта должен быть владелец (или команда).

Владелец отвечает за:
- Обновление runbook при изменении сервиса
- Мониторинг false positive rate
- Удаление/переделку если алерт не помогает

Мотивация простая: если алерт плохой — именно его будут будить ночью.

---

### Антипаттерн 5: Static thresholds вместо SLO

**Симптом:**
```yaml
# Плохо — 95% времени CPU выше 90% и ничего не ломается → алерт игнорируется
- alert: HighCPU
  expr: cpu_usage > 90
```

**Решение — SLO-based alerting (multi-burn-rate):**

```
SLO: 99.9% uptime → error budget: 43 минуты в месяц
```

| Burn rate | Интерпретация | Действие |
|-----------|---------------|----------|
| **14.4x** | Budget истощится за 2 дня | PAGE NOW |
| **6x** | Budget истощится за 5 дней | Создать тикет |
| **3x** | Budget истощится за 10 дней | Мониторить |
| **1x** | Плановое потребление | Смотреть дашборд |

**Пример Prometheus-алерта с burn rate:**
```yaml
- alert: ErrorBudgetBurnRateCritical
  ...
  labels:
    severity: critical
  annotations:
    summary: "Error budget горит в 14x — истощится за 2 дня"
    runbook: "https://wiki/runbooks/error-budget"
```

**Результат:** SLO-driven alerting снижает noise на ~80%, при этом реальные проблемы ловит лучше.

---

### Чеклист здорового алерта

- [ ] Относится к одному из 4 Golden Signals
- [ ] Имеет severity (DISASTER/CRITICAL/HIGH/WARNING)
- [ ] Для CRITICAL/DISASTER есть runbook (решается за 10 мин)
- [ ] Указан владелец (команда/человек)
- [ ] Порог на основе SLO/burn rate, а не static threshold
